...
def get_weather_by_coord(lat, lon):
    ...
    weather_request = f"https://api.openweathermap.org/data/3.0/onecall?lat={lat}&lon={lon}&exclude=hourly,daily,minutely&units=metric&lang=ru&appid={OW_API_KEY}"
...
def get_weather_overview_by_coord(lat, lon):
    ...
    print("get_weather_overview_by_coord")
    return f"{translated}"
